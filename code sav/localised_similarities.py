...
class LocalisedSimilaritiesClassifier(object):
    ...
    def _params_init(self,x_train, y_train , H ):
        ...
        if H is not None:
            ...
            self.H = np.ones((self.n_target, self.len_data)) #Résultat du boosting à l'étape n-1
    ...
    def fit(self, x, y, H=None):
        ...
        for element in x:
            ...
            liste_loss.append(self.optimal_loss())
        second_loss = min(liste_loss)
        indice_i = liste_loss.index(second_loss)
    ...
    def optimal_vector(self):
        ...
        sf_T[sf_T==0] = 1
        sf_F[sf_F==0] = 1
        a = 1 / 2 * (np.log(sf_T) - np.log(sf_F))  # Sert si l'on effectue l'algo de boosting pour attribuer les poids
        return a
    ...
    def compute_U(self):
        ...
        for element in range(self.len_data):
            u.append(self.W[:,element]*self.Y[:,element]/denom)
        u=np.array(u).T
        return np.dot(u.T,u)
